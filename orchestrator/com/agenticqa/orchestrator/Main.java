 ...
 * --config FILE config file (default: config/agenticqa.properties next to the
 * jar)
 * --dry-run skip the AI call; show scan + retrieval + route only
 * --debug-rag dump normalization + retrieved chunks; bypasses the intent gate
 *
...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // --debug-rag is a diagnostic mode, so it bypasses the gate on purpose.
        boolean debugRag = opts.containsKey("debug-rag");
        if (!debugRag) {
        ...
        if (opts.containsKey("rag"))
            config.ragEnabled = !"off".equalsIgnoreCase(opts.get("rag"));
        if (opts.containsKey("top-k"))
            config.topK = Integer.parseInt(opts.get("top-k"));

        // Project: --repo argument, or agenticqa.orchestrator.repoPath in the config
        // file.
